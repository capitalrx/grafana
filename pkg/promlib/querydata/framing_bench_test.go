...
// - go test -benchmem -run=^$ -bench ^BenchmarkExemplarJson$ github.com/capitalrx/grafana/pkg/promlib/querydata -memprofile memprofile.out -count 6 | tee old.txt
...
// when memory-profiling this benchmark, these commands are recommended:
// - go test -benchmem -run=^$ -bench ^BenchmarkRangeJson$ github.com/capitalrx/grafana/pkg/promlib/querydata -memprofile memprofile.out -count 6 | tee old.txt
// - go tool pprof -http=localhost:6061 memprofile.out
